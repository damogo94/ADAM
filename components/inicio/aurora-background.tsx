 ...
 *
 * Decisión consciente: la landing se aparta del monocromo B&W estricto del
 * resto de la app y usa una aurora de color (índigo → violeta → teal) que vira
 * al emerald semántico (= color de "positivo") al llegar a la sección del
 * veredicto. Los colores viven como CSS variables LOCALES de /inicio (no se
 * tocan los tokens globales del theme).
 *
...
const AURORA_VARS = {
  '--aurora-1': '#4338ca', // índigo
  '--aurora-2': '#7c3aed', // violeta
  '--aurora-3': '#0d9488', // teal
  '--aurora-verdict': '#10b981', // emerald (= "positivo")
} as CSSProperties;
...
export function AuroraBackground() {
  ...
  const drift1 = useTransform(scrollYProgress, [0, 1], [0.34, 0.16]);
  const drift2 = useTransform(scrollYProgress, [0, 1], [0.22, 0.32]);
  // El glow del veredicto aparece hacia el centro de la página (sección ejemplo).
  const verdictGlow = useTransform(scrollYProgress, [0.32, 0.5, 0.7], [0, 0.45, 0.14]);
  ...
  if (reduce) {
    ...
    return (
      <div
        ...
        style={AURORA_VARS}
      >
        <div className="absolute -left-[10%] top-[6%] h-[55vh] w-[55vh] rounded-full bg-[var(--aurora-1)] opacity-25 blur-[110px]" />
        <div className="absolute right-[2%] top-[40%] h-[50vh] w-[50vh] rounded-full bg-[var(--aurora-2)] opacity-20 blur-[110px]" />
        <div className="absolute left-[20%] bottom-[6%] h-[45vh] w-[45vh] rounded-full bg-[var(--aurora-3)] opacity-20 blur-[110px]" />
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <Blob
        ...
        style={{ x: x3, opacity: 0.22 }}
      />
      {/* Glow del veredicto — emerald, ligado al dato (dirección positiva). */}
      <Blob
