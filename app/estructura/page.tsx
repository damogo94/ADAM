...
function EstructuraInner() {
  ...
  return (
    <div className="min-h-screen bg-void pb-20 max-w-md mx-auto md:max-w-3xl">
      ...
      <div className="mx-4 mt-2 flex items-center justify-end gap-1.5 font-mono text-[11px] text-ink/55">
        <Link
          href="/analysis"
      ...
      {error && (
        ...
        >
          <div className="mb-0.5 font-sans text-[12px] font-bold tracking-wider text-ink/85">
            {error.title}
          </div>
          <div className="font-mono text-[12px] leading-snug text-ink/85">{error.message}</div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-3">
        {pasos.map((s) => (
          <div key={s.n} className="flex gap-2.5 rounded-[12px] border border-white/5 bg-white/[0.015] px-3 py-2.5">
            <span className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full border border-white/15 font-sans text-[12px] font-bold text-ink/80">
              {s.n}
            </span>
            <div className="min-w-0">
              <div className="font-mono text-[12px] font-medium leading-tight text-ink/85">{s.t}</div>
              <div className="mt-0.5 font-mono text-[11px] leading-snug text-ink/66">{s.d}</div>
            </div>
          </div>
        ))}
      </div>
